                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                    {prizes.map((prize: any) => {
                      ...
                      return (
                        <div 
                          key={prize.id} 
                          className={`bg-[#12161A] border ${rarity.border} rounded-2xl p-4 flex flex-col items-center text-center relative overflow-hidden group transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_20px_rgba(0,0,0,0.3)] shadow-lg`}
                        >
                          {/* Background Glow based on rarity */}
                          <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-24 h-24 ${rarity.bg} rounded-full blur-2xl opacity-40 group-hover:opacity-70 transition-opacity`} />
                          ...
                          <AnimatePresence>
                            {showOdds && (
                              <motion.div 
                                ...
                                className={`absolute top-2 right-2 text-[10px] sm:text-xs px-2 py-1 rounded-md font-bold border ${rarity.badge} backdrop-blur-md z-20 flex items-center gap-1`}
                              >
                          ...
                          <div className="w-20 h-20 sm:w-24 sm:h-24 flex items-center justify-center mb-3 mt-4 relative z-10">
                            {getPrizeImage(prize) ? (
                              <img 
                              ...
                              <div className={`w-16 h-16 rounded-full ${rarity.bg} flex items-center justify-center ${rarity.color} border border-white/5`}>
                          ...
                          <h4 className={`font-bold text-sm sm:text-base line-clamp-2 leading-tight relative z-10 ${rarity.color}`}>
                            {prize.name}
                          </h4>
                          
                          <div className="mt-2 h-6 flex items-center justify-center relative z-10">
                            {prize.prize_type === 'box' && prize.reference_value && Number(prize.reference_value) > 0 && (
                              <span className="text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded text-xs font-bold border border-emerald-500/20">
                                {formatCurrency(prize.reference_value)}
                              </span>
                            )}
                            {prize.prize_type === 'double_spins' && (
                              <span className="text-brand-400 bg-brand-500/10 px-2 py-0.5 rounded text-xs font-bold border border-brand-500/20">
                                {prize.double_spins_count}x R$ {Number(prize.double_spins_value).toFixed(2)}
                              </span>
                            )}
                          </div>
                        </div>
