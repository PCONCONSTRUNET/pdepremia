-- ============================================================
-- PREMIAJÁ — Migration 020: Provably Fair Double
-- ============================================================
...
-- Index for resolving bets quickly
CREATE INDEX IF NOT EXISTS double_bets_round_status_idx ON public.double_bets(round_id, status);
...
-- 2. RPC to Calculate Outcome & Resolve Bets
...
BEGIN
  ...
  -- Ensure we don't reveal outcome before betting phase ends (time % 15 >= 10)
  IF p_round_id = v_current_round_id AND mod(v_unix_time::int, 15) < 10 THEN
  ...
  -- Any user calling this function resolves the bets for everyone efficiently.
  FOR v_bet IN 
  ...
  -- 3. Return the cryptographic payload
  RETURN json_build_object(
    'round_id', p_round_id,
